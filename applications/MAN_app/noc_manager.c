 ...
 */
#include <api.h>
#include "SDN_includes/communication.h"
...
void send_ack_requester(int path_success, int source, int target, int requester_addr, int is_global, enum ReqMode mode){
	...
	//path_requester interaction
	//send(requester_addr, message, path_uint_size);
...
void initialize_noc_manager(unsigned int * msg){
	...
	//Replace the class repo id by the ID generated by global mapper
	//SetMyID(task_id);
