...
SC_MODULE(test_bench) {
	...
	~test_bench(){
		...
		for(int j=0;j<N_PE;j++){
			...
			sprintf(aux, "router %d port EAST flits %d ", j, MPSoC-> PE[j]->ps_router->wire_EAST);
			fprintf(fp,"%s",aux);
			sprintf(aux, "port WEST flits %d ",MPSoC-> PE[j] ->ps_router->wire_WEST);
			fprintf(fp,"%s",aux);
			sprintf(aux, "port NORTH flits %d ",MPSoC-> PE[j] ->ps_router->wire_NORTH);
			fprintf(fp,"%s",aux);
			sprintf(aux, "port SOUTH flits %d ",MPSoC-> PE[j] ->ps_router->wire_SOUTH);
			fprintf(fp,"%s",aux);
			sprintf(aux, "port LOCAL flits %d ",MPSoC-> PE[j] ->ps_router->wire_LOCAL);
			fprintf(fp,"%s",aux);
			sprintf(aux, "TOTAL flits %d ",MPSoC-> PE[j] ->ps_router->total_flits);
			fprintf(fp,"%s",aux);
			sprintf(aux, "NUMBER pck %d ",MPSoC-> PE[j] ->ps_router->mySwitchControl->number_pck);
			fprintf(fp,"%s",aux);


			sprintf(aux, "fluxo 0 = %d ", MPSoC-> PE[j]  ->ps_router->fluxo_0);
			fprintf(fp,"%s",aux);
			sprintf(aux, "fluxo 1 = %d ",MPSoC-> PE[j]->ps_router->fluxo_1);
			fprintf(fp,"%s",aux);
			sprintf(aux, "fluxo 2 =  %d ",MPSoC-> PE[j]->ps_router->fluxo_2);
			fprintf(fp,"%s",aux);
			sprintf(aux, "fluxo 3 = %d ",MPSoC-> PE[j]->ps_router->fluxo_3);
			fprintf(fp,"%s",aux);
			sprintf(aux, "fluxo 4 = %d ",MPSoC-> PE[j]->ps_router->fluxo_4);
			fprintf(fp,"%s",aux);
			sprintf(aux, "fluxo 5 = %d\n",MPSoC-> PE[j]->ps_router->fluxo_5);
			fprintf(fp,"%s",aux);
			fclose (fp);
