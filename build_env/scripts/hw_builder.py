...
def generate_hw_pkg( yaml_r ):
    ...
    if system_model_desc == "sc" or system_model_desc == "scmod":
        ...
        generate_to_systemc(io_list, io_name_list, yaml_r )
    
    elif system_model_desc == "vhdl":
       
        generate_to_vhdl(io_list, io_name_list, yaml_r )
        


def generate_to_systemc(io_list, io_name_list, yaml_r):
    ...
    cs_flit_width =     get_subnet_CS_flit_width(yaml_r)
    ...
    file_lines.append("#define SUBNETS_NUMBER      "+str(subnet_number)+"\n")
    file_lines.append("#define TAM_CS_FLIT          "+str(cs_flit_width)+"\n")
...
def generate_to_vhdl(io_list, io_name_list, yaml_r):
    ...
    #These two lines compute the logical memory addresses used by the mlite to index the pages at the local memory
    ...
    #Remove the lost ',' in the string_io_connections_vhdl
    ...
    file_lines.append("    constant SUBNETS_NUMBER           : integer := "+str(subnet_number)+";\n")
    file_lines.append("    constant TAM_CS_FLIT              : integer := "+str(cs_flit_width)+";\n")
