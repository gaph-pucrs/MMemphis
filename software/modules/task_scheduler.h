...
typedef struct {
	...
	unsigned int 	waiting_msg;		//!<Signals that tasks is waiting by a message delivery
	int				deadline_miss;		//!<Counts the number of deadline misses

	unsigned int 	tcb_ptr;			//!<Pointer for the respective task TCB into kernel slave

} Scheduling;

extern unsigned int cluster_master_address;	//!<Only extern variable in kernel slave scope. Store the cluster master XY address
...
void send_real_time_constraints(Scheduling *);
