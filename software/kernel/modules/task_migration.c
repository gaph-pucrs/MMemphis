 ...
 * \param new_proc New processor address of task
 * \param master_address Master address of the task
 * \param task_utilization Utilization of the task, if task is not RT, utilization is equal to 0
 */
void send_task_migrated(int migrated_task, int new_proc, unsigned int master_address){
	...
	//It is necessary to implement the TASK_TERMINATED_OTHER_CLUSTER when task is running outside of the cluster
	if (master_address != cluster_master_address){
...
#if TASK_MIGRATION_DEBUG
	puts("Real-time parameters: period "); puts(itoa(p->period)); puts(" deadline "); puts(itoa(p->deadline));
	putsv(" execution time ", p->execution_time);
#endif
 ...
 * \param migrate_tcb The TCB pointer of the task to be migrated
 */
void handle_migration_DATA_BSS(volatile ServiceHeader * p, TCB * migrate_tcb){
