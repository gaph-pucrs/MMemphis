...
unsigned int cluster_master_address;			//!<Only external variable in slave. Variable store the same information than cluster_master_address of kernel_slave.c
...
void send_deadline_miss_report(Scheduling * real_time_task){
	...
	//p->execution_time = real_time_task->execution_time;

	p->period = real_time_task->period;

	p->utilization = real_time_task->utilization;

	send_packet(p, 0, 0);

	putsv("#### -------->>>  Deadline miss, task = ", tcb_ptr->id);
}

/**Assembles and send a REAL_TIME_CONSTRANTS packet to the manager
 *  \param tcb_ptr Pointer of the scheduler variable into TCB structure
 *  \param task_utilz Total of CPU utilization
 */
void send_real_time_constraints(Scheduling * real_time_task){
	...
	p->service = RT_CONSTRANTS;

	p->task_ID = tcb_ptr->id;

	p->master_address = cluster_master_address;

	//p->execution_time = real_time_task->execution_time;

	p->period = real_time_task->period;

	p->utilization = real_time_task->utilization << 16 | cpu_utilization;

	send_packet(p, 0, 0);

	if (tcb_ptr->master_address != cluster_master_address){

		p = get_service_header_slot();

		p->header = cluster_master_address;

		p->service = RT_CONSTRANTS_OTHER_CLUSTER;
	...
	puts("Send real-time contraints, task utilization = "); puts(itoa(real_time_task->utilization)); putsv(", CPU utilization = ", cpu_utilization);
}
 ...
 *  \param is_migration Flag indicating when the function is called by a migration process
 */
void real_time_task(Scheduling * real_time_task, unsigned int period, int deadline, unsigned int execution_time){
	...
	real_time_task->utilization = ((execution_time*100) / period) + 1; //This + 1 is to correct unsigned integer padding

	cpu_utilization += real_time_task->utilization;

	real_time_task->deadline_miss = 0;
...
//#if DEBUG
	...
	putsv("\n ----> CPU utilization incremented: ", cpu_utilization);
//#endif
...
void inline update_real_time(unsigned int current_time){
	...
	for(int i=0; i<MAX_LOCAL_TASKS; i++){
		...
		if (task->status == RUNNING) {
			...
			//Check deadline miss during task execution
		...
		if (current_time >= (task->ready_time + task->period) ){
			...
			//Send deadline miss at the end of task period
			//if (task->deadline_miss){
			if (task->waiting_msg == 0 && task->remaining_exec_time > (task->execution_time/10)){
#if ENABLE_DEADLINE_MISS
			...
			((TCB *) task->tcb_ptr)->communication_time = current_time;
